...
function ajaxlogin(){
    ...
    {
        echo __("This account need verify email");
    }
    else
    {
...
function write_rating(){
    ...
    if(rating_exist($project_id,$post_type)){
        $result['success'] = false;
        $result['message'] = __("Invalid rating call");
        die(json_encode($result));
    }

    if($valid_check_stamp){
            ...
            $save_reviews->post_type = $post_type;
...
function ModelGetStateByCountryID()
{
    ...
    $result = ORM::for_table($config['db']['pre'].'subadmin1')
        ->select_many('id','code','asciiname','name')
        ->where(array(
            'country_code' => $country_code,
            'active' => 1
        ))
        ...
        if ($total > 0) {
            foreach ($result as $row) {
                ...
                if($count == 1)
                {
                    ...
                    $list .=  '<li class="selected"><a href="#" class="selectme" data-id="'.$state_id.'" data-name="'.$stateName.'" data-type="state"><strong>'.__("Region").' '.$stateName.'</strong></a></li>';
                }

                $list .= '<li class=""><a href="#" id="region'.$id.'" class="selectme" data-id="'.$id.'" data-name="'.$name.', '.$stateName.'" data-type="city"><span>'.$name.' <i class="fa fa-angle-right"></i></span></a></li>';
                $count++;
            }
...
function searchCityFromCountry()
{
    ...
    $total = ORM::for_table($config['db']['pre'].'cities')
        ...
        ->where_like('name', ''.$dataString.'%')
        ->count();

    $sql = "SELECT c.id, c.asciiname, c.name, c.subadmin1_code, s.name AS statename, co.asciiname AS countryname
FROM `".$config['db']['pre']."cities` AS c
INNER JOIN `".$config['db']['pre']."subadmin1` AS s ON s.code = c.subadmin1_code and s.active = 1
INNER JOIN `".$config['db']['pre']."countries` AS co ON co.code = c.country_code and co.active = 1
 WHERE (c.name like '%$dataString%' or c.asciiname like '%$dataString%') and c.active = 1
 ORDER BY
  CASE
...
function setFavAd()
{
    ...
    if ($num_rows == 0) {
        ...
        $insert_favads->post_type = $_POST['post_type'];
        $insert_favads->save();
...
function getsubcatbyidList()
{
    ...
    if (count($rows) > 0) {
        ...
        foreach ($rows as $info) {
            ...
            echo '<li data-ajax-subcatid="'.$sub_id.'" class="'.$selected_text.'"><a href="#">'.$name.'</a></li>';
        }
