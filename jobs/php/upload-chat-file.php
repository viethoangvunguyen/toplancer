...
if (!$chunks || $chunk == $chunks - 1) {
    ...
    $query = "insert into `".$config['db']['pre']."messages`
        (message_date,from_id,to_id,message_content,message_type,post_id,post_type) values ".
        "('".$GLOBALS['timenow']."', $from_user_id, $to_id,'".mysqli_real_escape_string($con,$message_content)."','file','".mysqli_real_escape_string($con,$post_id)."','".mysqli_real_escape_string($con,$post_type)."')";
    $con->query($query);
