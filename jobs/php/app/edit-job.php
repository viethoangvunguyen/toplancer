...
function ajax_edit_advertise(){
    ...
    if(isset($_POST['submit'])) {
        ...
        if($config['company_enable']) {
            if (!count($errors) > 0) {
                if (isset($_FILES['company_logo']['name']) && $_FILES['company_logo']['name'] != ""){
                    ...
                    $result = quick_file_upload('company_logo',$target_dir);
                    if($result['success']){
                        $company_logo = $result['file_name'];
                        resizeImage(200, $target_dir . $company_logo, $target_dir . $company_logo);
                    }else{
        ...
        if($config['job_image_field']) {
            if (!count($errors) > 0) {
                if (isset($_FILES['job_image']['name']) && $_FILES['job_image']['name'] != ""){
                    $target_dir = ROOTPATH . "/storage/products/";
                    $result = quick_file_upload('job_image',$target_dir);
                    if($result['success']){
                        $job_image = $result['file_name'];
                        resizeImage(200, $target_dir . $job_image, $target_dir . $job_image);
                    }else{
                        $errors[]['message'] = $result['error'];
                    }
                }
        ...
        if (!count($errors) > 0) {
            if (checkloggedin()) {
                ...
                if($item_status == "pending" or $config['post_auto_approve'] == 1)
                {
                    ...
                    $item_edit->set('url_map', $_POST['url_map']);
                    $item_edit->save();
                }
                elseif($item_status == "active" or $item_status == "softreject" or $item_status == "expire")
                {
                    ...
                    $item_insrt->url_map = $_POST['url_map'];
                    $item_insrt->save();
                }
...
if(checkloggedin()) {
    ...
    if(check_valid_author($_GET['id'])){
        ...
        global $errors, $catid,$catName, $subcatid,$subcatName, $title, $description, $negotiable, $phone, $hide_phone, $tags, $cityid, $mapLat, $mapLong, $seller_name, $seller_email;
        ...
        if (!empty($info)) {
            ...
            HtmlTemplate::display('job-edit', array(
                ...
                'categories' => get_maincategory('default',$catid),
