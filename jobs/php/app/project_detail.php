...
if ($num_rows > 0) {
    ...
    $user_country_name = isset($user_country)? get_countryName_by_code($info['user_country']) : "";
    ...
    $bids_count;
    $bid_total_created = ORM::for_table($config['db']['pre'] . 'bids')
        ->where('user_id', $_SESSION['user']['id'])
        ->count();
    if($bids_count >= $bid_total_created){
        //echo "available";
    }else{
        //echo "not available";
    }
    //check when both value are equal then cant post new bid.
...
foreach($result as $info){
    ...
    $quickchat_url = $link['MESSAGE']."/?postid=$postid&posttype=project&userid=$qcuserid";
...
HtmlTemplate::display('project_detail', array(
    ...
    'error' => $error,
    'item_favorite' => check_product_favorite($item_id,'project'),
