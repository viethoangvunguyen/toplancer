...
//Banner AD
$result_banner= ORM::for_table($config['db']['pre'].'qbm_banners')
...
if (count($result_banner) > 0) {
    ...
    foreach ($result_banner as $info2 ) {
        if ($now < $info2['time_expired']) {
            $item_b[$info2['id']]['id'] = $info2['id'];
            $item_b[$info2['id']]['title'] = $info2['title'];
            $item_b[$info2['id']]['url'] = $info2['url'];
            $item_b[$info2['id']]['file'] = $info2['file'];
        }
    }
}
//Banner AD
...
//Popup Services
$where .= "inner JOIN `ql_user` u on u.id = p.freelancer_id where p.status = 'under_development';";
$query = "SELECT p.*, u.username, u.name as fullname, u.image as user_image
FROM `".$config['db']['pre']."project`  p $where";
...
if (count($result) > 0) {
    ...
    foreach ($result as $info1 ) {
        ...
        $item[$info1['id']]['link'] = $link['PROJECT'].'/' . $info1['id'] . '/'.$pro_url;
    }
}
//Popup Services
...
//Feature Jobs
$item3 = array();
$item3 = get_projects_feature("","open",false,1,$limit,"p.id",true,true,"DESC");
//Feature Jobs
//Recommend Jobs
$item4 = array();
$user_id_current = $_SESSION['user']['id'];
if($user_id_current){
    $item4 = get_projects_recommend($user_id_current,"open",false,1,$limit,"p.id",true,true,"DESC");
}
//Recommend Jobs
$post_type = "default";
...
$total_jobs = $count = ORM::for_table($config['db']['pre'].'product')->where('status','active')->count();
$total_projects = $count = ORM::for_table($config['db']['pre'].'project')->where('status','open')->count();

$community_earning = 0;
$rows = ORM::for_table($config['db']['pre'].'milestone')
    ->select_many('amount')
    ->where('status', 'paid')
    ->find_many();
