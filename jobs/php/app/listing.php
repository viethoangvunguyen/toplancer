...
LEFT JOIN `".$config['db']['pre']."companies` c on p.company_id = c.id
 WHERE p.status = 'active' AND p.hide = '0' ";
    ...
    $query = "SELECT p.*,u.group_id, c.name company_name, c.logo company_image FROM `".$config['db']['pre']."product` as p
    LEFT JOIN `".$config['db']['pre']."companies` c on p.company_id = c.id 
...
if ($result) {
    foreach ($result as $info)
    {
        ...
        $items[$info['id']]['favorite'] = check_product_favorite($info['id'],'job');
        ...
        if(check_user_upgrades($info['user_id']))
        {
            ...
            $items[$info['id']]['sub_title'] = $sub_info['name'];
            $items[$info['id']]['sub_image'] = $sub_info['badge'];
        }else{
