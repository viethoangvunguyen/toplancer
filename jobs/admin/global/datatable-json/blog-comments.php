...
foreach ($queryRecords as $row) {
    ...
    $status = '';
    if ($row['active'] == "0") {
        $status = '<span class="label label-warning">Chưa được phê duyệt</span>';
    } elseif ($row['active'] == "1") {
        $status = '<span class="label label-success">Đã được phê duyệt</span>';
    }
    ...
    $row6 = '<td class="text-center">
                <div class="btn-group">'.
        ($row['active'] == "0"?'<a href="#" title="Phê duyệt" class="btn btn-xs btn-success item-approve" data-ajax-action="approveComment"> <i class="ion-android-done"></i> </a>':'').'
                    <a href="#" title="Xóa" class="btn btn-xs btn-default item-js-delete" data-ajax-action="deleteComment"><i class="ion-close"></i></a>
                </div>
            </td>';
    $value = array(
        "DT_RowId" => $id,
