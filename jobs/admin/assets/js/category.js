jQuery(function($) {
    ...
    $new_subcategory_form.on('submit', function() {
        ...
        $.post(ajaxurl+'?action=addSubCat&mainid='+id, data, function(response) {
            if(response != 0){
                ...
                quickadAlert({success: ['Tạo thành công.']});
    ...
    $(document)
        ...
        .on('click', '.quickad-category-item', function(e) {
            ...
            $.get(ajaxurl, {action:'getSubCat', post_type: post_type, category_id: $clicked.data('category-id')}, function(response) {
                if ( response != 0 ) {
                    $('.quickad-category-title').text($clicked.find('.displayed-value').text());
                    $('#ab-services-list').html(response);
                }else{
                    $('#ab-services-list').html('<h3>Không tìm thấy danh mục con.</h3>');
                }
            ...
            if (confirm('Bạn có chắc không?')) {
                ...
                $.post(ajaxurl+'?action=delSubCat', data, function(response) {
                    if(response != 0) {
                        ...
                        quickadAlert({success: ['Xóa Thành Công.']});
                    }else{
                        $('#quickad-delete').removeClass('bookme-progress');
                        quickadAlert({error: ['Có vấn đề khi xóa, vui lòng thử lại.']});
                    }
                });
            }
        });
        ...
        update : function( event, ui ) {
            ...
            $.ajax({
                ...
                success: function (response, textStatus, jqXHR) {
                    ...
                    if(response != 0) {
                        quickadAlert({success: ['Thứ tự danh mục thay đổi thành công.']});
                    }else{
                        quickadAlert({error: ['Có vấn đề khi thay đổi thứ tự, vui lòng thử lại.']});
                    }
                }
            });
        }
                ...
                update : function( event, ui ) {
                    ...
                    $.ajax({
                        ...
                        success: function (response, textStatus, jqXHR) {
                            // Remove Ads item from DOM.
                            if(response != 0) {
                                quickadAlert({success: ['Thứ tự danh mục con thay đổi thành công..']});
                            }else{
                                quickadAlert({error: ['Có vấn đề khi thay đổi thứ tự, vui lòng thử lại.']});
                            }
                        }
