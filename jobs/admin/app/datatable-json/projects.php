...
foreach ($queryRecords as $row) {
    ...
    $status = '';
    if ($ad_status == "open"){
        $status = '<span class="label label-success">Đang Mở</span>';
    }
    ...
    {
        $status = '<span class="label label-info">Đang Tiến Hành</span>';
    }
    elseif($ad_status == "completed")
    {
        $status = '<span class="label label-primary">Hoàn Thành</span>';
    }
    elseif($ad_status == "incomplete")
    {
        $status = '<span class="label label-danger">Không Hoàn Thành</span>';
    }
    else{
        $status = '<span class="label label-danger">Đóng</span>';
    }
    ...
    $row3 = '<td class="hidden-xs">'.$bids_count.' Phiếu</td>';
    $row4 = '<td class="hidden-xs"><a class="label label-warning" href="milestone.php?id='.$id.'" target="_blank">'.$milestone_count.' Cột Mốc</a></td>';
