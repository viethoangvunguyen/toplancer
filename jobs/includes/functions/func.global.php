...
function overall_header($page_title = '', $meta_desc = '', $meta_image = '', $meta_article = false)
{
    ...
    if (isset($_SESSION['user']['id'])) {
        ...
        if (check_user_upgrades($userdata['id'])) {
            $sub_info = get_user_membership_detail($userdata['id']);
            $badge = $sub_info['badge'];
        } else {
            $badge = '';
        }
    } else {
    ...
    foreach ($result as $info) {
        ...
        $name = $info['name'];
        if ($count == 1) {
            $states[$count]['tpl'] =  '<li class="selected"><a class="selectme" data-id="' . $country_code . '" data-name="' . __('All') . '" data-type="country"><strong>' . __('All') . '</strong></a></li>';
        }
...
function check_update_product()
{
    ...
    $result = ORM::for_table($config['db']['pre'] . 'product')
        ->where('status', 'active')
        ->where_raw("expire_date <= $currentTimestamp")
        ->order_by_asc('id')
        ->find_many();

    // Vòng lặp qua kết quả trả về
...
function payment_success_save_detail($access_token)
{
    ...
    if ($payment_type == "subscr") {
        ...
        if (check_valid_author($item_pro_id)) {
            ...
            message(__("Success"), __("Payment Successful"), $link['TRANSACTION']);
            exit();
        } else if (check_valid_project($item_pro_id)) {
            ...
            $highlight_exp_date = date('Y-m-d H:i:s', $highlight_exp_date);
            if ($item_featured == '1') {
                $featured_insert = ORM::for_table($config['db']['pre'] . 'project')->find_one($item_pro_id);
                $featured_insert->featured = '1';
                $featured_insert->featured_exp_date = $featured_exp_date;
                $featured_insert->save();
            }
            if ($item_urgent == '1') {
                ...
                $urgent_insert = ORM::for_table($config['db']['pre'] . 'project')->find_one($item_pro_id);
                $urgent_insert->urgent = '1';
                $urgent_insert->urgent_exp_date = $urgent_exp_date;
                $urgent_insert->save();
            }
            if ($item_highlight == '1') {
                ...
                $highlight_insert = ORM::for_table($config['db']['pre'] . 'project')->find_one($item_pro_id);
                $highlight_insert->highlight = '1';
                $highlight_insert->highlight_exp_date = $highlight_exp_date;
                $highlight_insert->save();
            }
