...
function check_valid_project($project_id){
...
function get_categories_dropdown($post_type = 'default'){
    ...
    foreach($result1 as $info1){
        ...
        $result = ORM::for_table($config['db']['pre'].'catagory_sub')
            ...
            ->find_many();
        if(count($result) > 0){
            $dropdown .= '<li><a href="#" data-ajax-id="'.$cat_id.'" data-cat-type="maincat">'.$icon.' '.$catname.'</a><span class="dropdown-arrow"><i class="fa fa-angle-right"></i></span><ul style="overflow-y: scroll">';
        }else{
 ...
 * @return array
 */
function get_projects_recommend($userid, $status=null, $premium=false, $page=null, $limit=null, $sort="p.id", $location=false, $order=false, $sort_order="DESC"){

    global $config,$lang,$link;
    $where = '';
    $item = array();
    if($userid != null){
        $data = ORM::for_table('ql_user')->where_equal('id', $userid)->find_many();
        ...
        if($count > 0) {
            foreach($data as $d){
                ...
                if($d['subcategory']){
                    if($where == '')
                        $where .= "where p.sub_category = '".$d['subcategory']."'";
                    else
                        $where .= " AND p.sub_category = '".$d['subcategory']."'";
                } 
                if($d['category']){
                    if($where == '')
                        $where .= "where p.category = '".$d['category']."'";
                    else
                        $where .= " AND p.category = '".$d['category']."'";
                }
            }
        }
    }
...
function get_projects_feature($userid=null, $status=null, $premium=false, $page=null, $limit=null, $sort="p.id", $location=false, $order=false, $sort_order="DESC"){
    ...
    $query = "SELECT p.*
FROM `".$config['db']['pre']."project`  p
$where ORDER BY p.view DESC ,$order_by $pagelimit";
    $result = ORM::for_table($config['db']['pre'].'project')->raw_query($query)->find_many();
 ...
 * @return array
 */
function get_items_recommend($userid, $status=null, $premium=false, $page=null, $limit=null, $sort="p.id", $location=false, $order=false, $sort_order="DESC") {
...
function get_items_feature($userid=null, $status=null, $premium=false, $page=null, $limit=null, $sort="p.id", $location=false, $order=false, $sort_order="DESC"){
...
$where ORDER BY p.view DESC, $order_by $pagelimit";
    ...
    if(count($result) > 0){
        ...
        $where = str_replace("AND (p.featured = '1' or p.urgent = '1' or p.highlight = '1')", "", $where);
        $querys = "SELECT p.*, c.id company_id, c.name company_name, c.logo company_image
        FROM `".$config['db']['pre']."product`  p
        LEFT JOIN `".$config['db']['pre']."companies` c on p.company_id = c.id 
        $where ORDER BY p.view DESC, $order_by $pagelimit";
        $result = ORM::for_table($config['db']['pre'].'product')->raw_query($querys)->find_many();
    }
    if ($result) {
        foreach ($result as $info)
        {
...
function renew_item_by_userid($userid=null){
    ...
    if($group_id > 0) {
        ...
        $group_info = get_user_membership_detail($userid);
...
function minus_balance($user_id, $amount){
    ...
    $trans_insert->product_name = 'Phí hoa hồng';
    ...
    $trans_insert->transaction_description = 'Hoa hồng dự án';
...
/**
 * Minus amount from user balance for banner adv
 *
 * @param int $user_id
 * @param float $amount
 * @param int $banner_id
 */
function minus_balance_adv($user_id, $amount, $banner_id){
    global $config;
    ...
    $now = time();
    $ip = encode_ip($_SERVER, $_ENV);
    $trans_insert = ORM::for_table($config['db']['pre'].'qbm_transactions')->find_one($banner_id);
    $trans_insert->set('payment_status','success');
    $trans_insert->save();
}

/**
 * Add amount to user balance for banner adv
 *
 * @param int $user_id
 * @param float $amount
 * @param int $banner_id
 */
function add_balance_adv($user_id, $amount, $banner_id){
    global $config;
...
/**
 * Minus amount from user balance for withdraw
 *
 * @param int $user_id
 * @param float $amount
 */
function minus_balance_withdraw($user_id, $amount){
    global $config;
...
/**
 * Add amount to user balance for withdraw
 *
 * @param int $user_id
 * @param float $amount
 */
function add_balance_withdraw($user_id, $amount, $type){
    global $config;
    ...
    $ip = encode_ip($_SERVER, $_ENV);
    $trans_insert = ORM::for_table($config['db']['pre'].'transaction')->create();
    $trans_insert->product_name = 'Yêu cầu rút tiền';
    $trans_insert->product_id = '';
    $trans_insert->user_id = $user_id;
