            ...
            <div class="row">
                ...
                <div class="col-xl-12">
                    <div class="dashboard-box margin-top-0">
                        <!-- Headline -->
                        <div class="headline">
                            <h3><i class="icon-feather-users"></i> <?php _e("Applied Users") ?> &ndash;
                                <?php _esc($product_name)?></h3>
                        </div>
                        <?php
                if(!$totalitem) {
                    ...
                    <div class="listings-container margin-top-35">
                        ...
                        <div class="job-listing">
                            <div class="job-listing-details">
                                <div class="job-listing-company-logo">
                                    <a href="<?php url("PROFILE") ?>/<?php _esc($item['username'])?>">
                                        <img src="<?php _esc($config['site_url'])?>storage/profile/<?php _esc($item['image'])?>"
                                            alt="<?php _esc($item['name'])?>">
                                    </a>
                                </div>
                                <div class="job-listing-description">
                                    <h3 class="job-listing-title"><a
                                            href="<?php url("PROFILE") ?>/<?php _esc($item['username'])?>"><?php _esc($item['name'])?></a>
                                        <?php
                                    if($item['sex'] == 'Male') {
                                    ...
                                    <p class="job-listing-text read-more-toggle"
                                        data-read-more="<?php _e("Read more") ?>"
                                        data-read-less="<?php _e("Read less") ?>"><?php _esc($item['description'])?></p>
                                </div>
                                <?php if($item['resume'] != ""){ ?>
                            ...
                            <div class="job-listing-footer with-icon">
                                ...
                                <span
                                    class="fav-icon set-user-fav <?php if($item['favorite'] == '1') { echo 'added'; }?>"
                                    data-favuser-id="<?php _esc($item['user_id'])?>"
                                    data-userid="<?php _esc($user_id)?>" data-action="setFavUser"></span>
                                ...
                                <div class="pagination-container margin-top-20 margin-bottom-60">
                                    <nav class="pagination">
                                        ...
                                        foreach($pages as $page) {
                                            if ($page['current'] == 0){
                                                ?>
                                            <li><a href="<?php _esc($page['link'])?>"><?php _esc($page['title'])?></a>
                                            </li>
                                            <?php }else{
                                                ?>
