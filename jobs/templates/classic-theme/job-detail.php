                    ...
                    <?php if ($is_login) {
                        if ($usertype == 'user') {
                            if ($already_applied == '1') {
                                ...
                                echo '<a href="#apply-now-dialog" class="apply-dialog-button button ripple-effect popup-with-zoom-anim">' . __("Apply Now") . ' <i class="icon-feather-arrow-right"></i></a>';
                            }
                        }
                    } else {
    ...
    <div class="row">
                    ...
                    <?php if ($item_salary_min != "0") { ?>
                        <div class="col-md-6">
                            <div class="job-property">
                                ...
                                <h5><?php _esc($item_salary_min) ?> - <?php _esc($item_salary_max) ?> /
                                    <?php _esc($item_salary_type) ?>
        ...
        <div class="col-xl-4 col-lg-4">
            <div class="sidebar-container">
                            ...
                            <?php if ($is_login) {
                                if ($usertype == 'user') {
                                    ...
                                    if ($item_application_url != '') {
                                        echo '<a href="' . _esc($item_application_url, false) . '" class="button ripple-effect" target="_blank" rel="nofollow">' . __("Trang web công ty") . ' <i class="icon-feather-arrow-right"></i></a>';
                                    } else {
                                        if ($already_applied == '1') {
                                            echo '<button class="button green disabled" disabled><i class="icon-feather-check"></i> ' . __("Already Applied") . '</button>';
                                        } else {
                                    ...
                                    data-chatid="' . _esc($item_authorid, false) . '_' . _esc($item_id, false) . '_job"
                ...
                <!-- Sidebar Widget -->
                <div class="sidebar-widget">
                    <h3><?php _e("Lưu tin") ?></h3>

                    <?php if ($usertype == 'user') { ?>
                        <!-- Bookmark Button -->
                        <button class="bookmark-button fav-button margin-bottom-25 set-item-fav <?php if ($item_favorite == '1') {
                                                                                                    echo 'added';
                                                                                                } ?>" data-item-id="<?php _esc($item_id) ?>" data-userid="<?php _esc($user_id) ?>" data-post-type="job" data-action="setFavAd">
                            <span class="bookmark-icon"></span>
                            <span class="fav-text"><?php _e("Yêu thích") ?></span>
                            <span class="added-text"><?php _e("Bỏ yêu thích") ?></span>
                        </button>
                    <?php } ?>
                    <!-- Copy URL -->
                    <div class="copy-url">
                ...
                <div class="sidebar-widget">
                    ...
                    <ul class="related-links">
                        <?php if ($config['company_enable']) { ?>
                            <li>
                                <a href="<?php _esc($company_link) ?>#all-jobs"><i class="la la-suitcase"></i>
                                    <?php _e("More jobs by") ?> <?php _esc($company_name) ?></a>
                            </li>
                        <?php } ?>
                        <li>
                            <a href="<?php _esc($user_link) ?>#all-jobs"><i class="la la-user"></i>
                                <?php _e("More jobs by") ?> <?php _esc($user_name) ?></a>
                        </li>
                        <li>
                            <a href="<?php url("REPORT") ?>"><i class="la la-exclamation-triangle"></i>
                                <?php _e("Report this job") ?></a>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <form method="post" action="" accept-charset="UTF-8" enctype="multipart/form-data">
                    ...
                    <?php if ($config['resume_enable']) { ?>
                                ...
                                <?php foreach ($resumes as $resume) { ?>
                                    <li>
                                        <div class="radio">
                                            ...
                                            <label for="resume-<?php _esc($resume['id']) ?>"><span class="radio-label"></span>
                                                <?php _esc($resume['name']) ?> - <a href="<?php _esc($config['site_url']) ?>storage/resumes/<?php _esc($resume['filename']) ?>" download=""><?php _e("Download") ?></a></label>
                                        </div>
                                    </li>
                                <?php } ?>
                        ...
                        <div class="uploadButton resume-upload-button">
                            <span class="uploadButton-file-name"><?php _e("Chưa có CV. Vui lòng tải CV lên.") ?></span>
                        </div>
                    <?php } ?>
...
?>
        ...
        <script type='text/javascript' src='//maps.google.com/maps/api/js?key=<?php _esc($config['gmap_api_key']) ?>&#038;libraries=places%2Cgeometry&#038;ver=2.2.1'>
        </script>
